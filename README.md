...
- **Live office floor + Mosaic** — every session and sub-agent as an animated tile, with parent→child packet flow; agents wander to the water cooler when idle and clock out when done.
...
- **📋 Task queue · 📰 Ship digest · Routines** — line up goals per project, see what actually got done, schedule headless morning briefings.
...
- **📱 Anywhere** — installable PWA for your phone, Telegram replies, native desktop toasts, smart-light ambient alerts.
