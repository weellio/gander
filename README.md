...
- **🤖 Process robots on the floor** — the background processes your sessions leave running appear as little robots: parked at the back wall of their session's room when the owner is on the floor, otherwise in a **SERVER ROOM** below it (plugin runtimes like the Telegram plugin's `bun.exe` pairs, children of sessions opened outside Gander, port-holders). Colour = what it is (indigo plugin · green claude-child · amber pulsing = orphaned, parent gone); a port-holding bot wears its `:port`. **Click a bot** for its details — what it's linked to, uptime, ports — a plain-language "is it safe to kill?" hint, and a Kill button. Fed by the same attribution engine as the Processes panel, scanned lazily in the background only while the dashboard is open.
