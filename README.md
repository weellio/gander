...
- **Can I answer prompts from the dashboard?** Yes — the **⌨ quick-keys** type a keystroke into the session's window (found by PID). It steals focus and types into whatever's focused there, so keep the **Claude terminal focused** in that window.
- **"'claude' is not recognized" when I ▶ Start.** Your `claude` CLI isn't on PATH — set its full path in **Settings → New session options** (`where claude` / `which claude`).
