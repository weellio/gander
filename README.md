...
- **📋 Task queue · 📰 Ship digest · Routines** — line up goals per project: **⎇ worktree isolation** (parallel tasks in one project), a **🧪 test gate** (only green branches merge), a **👀 review-before-merge** hold (diff + approve / request changes from the rail), `then:` chaining, a **✓ definition of done**, **⊘ competing candidates** (fan a hard goal into N attempts, pick the winner), and it **holds while your plan is rate-limited**. Plus the ship digest and headless morning briefings.
