...
- **🖥 Claude Desktop on the floor** — if the Anthropic desktop app is installed (classic or Microsoft Store build), a **Claude Desktop tile** appears automatically: running/closed state, live **MCP tool-call activity** (tailed from its local logs, with the tool name), and agent-mode activity. Its conversations live on claude.ai, so the tile is **view-only** — but you can finally *see* that Desktop is busy, from the same floor (or your phone). Disable with `{ "desktopWatch": false }`. *(Claude in Chrome leaves no local trace — verified — so it can't be watched; that story is in the FAQ.)*
